...
namespace BNJMO
{
    public class BNetCodeBehaviour : NetworkBehaviour
    {
        ...
        protected void DrawArrow(Vector3 pos, Vector3 direction, float arrowHeadLength = 0.25f,
        ...
        /// <param name="objectToCheck"> object to check </param>
        /// <returns> True if the given object has a valid reference, otherwise false</returns>
        protected bool IS_NOT_VALID<O>(O objectToCheck, bool reverseLogCondition = false)
        ...
        protected bool ARE_EQUAL<O>(O variable1, O variable2, bool reverseLogCondition = false)
        {
            ...
            {
                if (!reverseLogCondition)
                    LogConsoleWarning("Two variables of type <color=cyan>" + typeof(O) +
                                      "</color> are not equal! " + variable1 + " - " + variable2);
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        protected bool IS_GREATER_OR_EQUAL<T>(T value1, T value2, bool reverseLogCondition = false)
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        protected bool IS_LESS_OR_EQUAL<T>(T value1, T value2, bool reverseLogCondition = false)
            where T : IComparable<T>
        {
            int comparisonResult = value1.CompareTo(value2);
            if (comparisonResult < 0)
        ...
        /// <param name="reverseLogCondition"></param>
        /// <returns></returns>
        protected bool ARE_ENUMS_EQUAL<E>(E enumToCheck1, E enumToCheck2, bool reverseLogCondition = false)
            where E : Enum
        {
        ...
        /// <param name="reverseLogCondition"></param>
        /// <returns></returns>
        protected bool ARE_ENUMS_NOT_EQUAL<E>(E enumToCheck1, E enumToCheck2, bool reverseLogCondition = false)
            where E : Enum
        {
            if (enumToCheck1.ToString().Equals(enumToCheck2.ToString()) == true)
            {
                if (!reverseLogCondition)
                    LogConsoleWarning("Two enums of type <color=cyan>" + typeof(E) +
                                      "</color> are NOT different! ");
        ...
        protected bool IS_KEY_CONTAINED<K, V>(Dictionary<K, V> dictionary, K key, bool reverseLogCondition = false)
        {
            if (dictionary.ContainsKey(key) == false)
            {
                ...
                if (!reverseLogCondition)
                    LogConsoleWarning("A dictionary with keys type <color=cyan>" + typeof(K) +
                                      "</color> doens't contain the key '" + key + "' ! ");
            ...
            if (reverseLogCondition)
                LogConsoleWarning("A dictionary with keys type <color=cyan>" + typeof(K) +
                                  "</color> does contain the key '" + key + "' ! ");
        ...
        protected bool IS_KEY_NOT_CONTAINED<K, V>(Dictionary<K, V> dictionary, K key,
            bool reverseLogCondition = false)
        {
            if (dictionary.ContainsKey(key) == true)
            {
                ...
                if (!reverseLogCondition)
                    LogConsoleWarning("A dictionary with keys type <color=cyan>" + typeof(K) +
                                      "</color> does already contain the key '" + key + "' ! ");
            ...
            if (reverseLogCondition)
                LogConsoleWarning("A dictionary with keys type <color=cyan>" + typeof(K) +
                                  "</color> does not contain the key '" + key + "' ! ");
        ...
        protected bool IS_VALUE_CONTAINED<K, V>(Dictionary<K, V> dictionary, V value)
        {
            ...
            {
                LogConsoleWarning("A dictionary with values type <color=cyan>" + typeof(V) +
                                  "</color> doens't contain the value '" + value + "' ! ");
                return false;
            }
        ...
        protected bool IS_VALUE_NOT_CONTAINED<K, V>(Dictionary<K, V> dictionary, V value)
        {
            ...
            {
                LogConsoleWarning("A dictionary with values type <color=cyan>" + typeof(V) +
                                  "</color> does contain already the value '" + value + "' ! ");
                return false;
            }
        ...
        protected bool IS_VALUE_CONTAINED<V>(List<V> list, V value)
        {
            ...
            {
                LogConsoleWarning("A list with values type <color=cyan>" + typeof(V) +
                                  "</color> doens't contain the value '" + value + "' ! ");
                return false;
            }
        ...
        protected bool IS_VALUE_NOT_CONTAINED<V>(V[] array, V value)
        {
            ...
            {
                LogConsoleWarning("A list with valuess type <color=cyan>" + typeof(V) +
                                  "</color> does contain already the value '" + value + "' ! ");
                return false;
            }
        ...
        protected bool IS_VALID_INDEX<V>(V[] array, int index, bool reverseLogCondition = false)
        {
            ...
            {
                if (!reverseLogCondition)
                    LogConsoleWarning("'" + index +
                                      "' is not a valid index for the array of type type <color=cyan>" + typeof(V) +
                                      "</color> ! ");
            ...
            if (reverseLogCondition)
                LogConsoleWarning("'" + index + "' is a valid index for the array of type type <color=cyan>" +
                                  typeof(V) + "</color> ! ");
        ...
        protected bool IS_NOT_VALID_INDEX<V>(V[] array, int index, bool reverseLogCondition = false)
        {
            ...
            if (reverseLogCondition)
                LogConsoleWarning("'" + index + "' is not a valid index for the array of type type <color=cyan>" +
                                  typeof(V) + "</color> ! ");
                ...
                if (prefabStage != null)
                {
                    if (PrefabUtility.IsPartOfPrefabInstance(gameObject))
                    {
                        ...
                        var nestedPrefabPath =
                            AssetDatabase.GetAssetPath(PrefabUtility.GetCorrespondingObjectFromSource(gameObject));
        ...
        public void LogPrefabInformation()
        {
            ...
            else
            {
                ...
                GameObject originalPrefabAssetObject =
                    PrefabUtility.GetCorrespondingObjectFromOriginalSource(gameObject);
                stringBuilder.AppendFormat(
...
",
        ...
        protected IEnumerator WaitCoroutine<A, B, C>(float waitDuration, A arg1, B arg2, C arg3,
            Action<A, B, C> action)
        {
            yield return new WaitForSeconds(waitDuration);
        ...
        /* 4 Parameters */
        protected void Wait<A, B, C, D>(float waitDuration, A arg1, B arg2, C arg3, D arg4,
            Action<A, B, C, D> action)
        {
            StartCoroutine(WaitCoroutine(waitDuration, arg1, arg2, arg3, arg4, action));
        }

        protected IEnumerator WaitCoroutine<A, B, C, D>(float waitDuration, A arg1, B arg2, C arg3, D arg4,
            Action<A, B, C, D> action)
        {
            yield return new WaitForSeconds(waitDuration);
        ...
        /* 5 Parameters */
        protected void Wait<A, B, C, D, E>(float waitDuration, A arg1, B arg2, C arg3, D arg4, E arg5,
            Action<A, B, C, D, E> action)
        {
            StartCoroutine(WaitCoroutine(waitDuration, arg1, arg2, arg3, arg4, arg5, action));
        }

        protected IEnumerator WaitCoroutine<A, B, C, D, E>(float waitDuration, A arg1, B arg2, C arg3, D arg4,
            E arg5, Action<A, B, C, D, E> action)
        {
            yield return new WaitForSeconds(waitDuration);
        ...
        public T[] GetComponentsWithCheck<T>(bool logWarningMessageIfNoneFound = true)
        {
            ...
            if (logWarningMessageIfNoneFound
                && result.Length == 0)
            {
                LogConsoleWarning("Not any component of type <color=cyan>" + typeof(T) + "</color> not found on '" +
                                  name + "'");
            }
        ...
        public T GetComponentInParents<T>(bool excludeSelf = true, bool logWarningMessageIfNoneFound = false)
        {
            ...
            if (logWarningMessageIfNoneFound
                && result == null)
            {
                LogConsoleWarning("Component of type <color=cyan>" + typeof(T) +
                                  "</color> not found in the parents of '" + name + "'");
            }
        ...
        public T GetComponentInDirectParent<T>(bool logWarningMessageIfNoneFound = false)
        {
            ...
            {
                LogConsoleWarning("Component of type <color=cyan>" + typeof(T) +
                                  "</color> not found in the direct parent of '" + name + "'");
            }
        ...
        /// <returns> true if invoked, otherwise false if not bound </returns>
        protected bool InvokeEventIfBound<A, B, C, D>(Action<A, B, C, D> eventToInvoke, A arg1, B arg2, C arg3,
            D arg4)
        {
            if (eventToInvoke != null)
            {
        ...
        /// <returns> true if invoked, otherwise false if not bound </returns>
        protected bool InvokeEventIfBound<A, B, C, D, E>(Action<A, B, C, D, E> eventToInvoke, A arg1, B arg2,
